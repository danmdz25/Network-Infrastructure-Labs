...
* **Documentação In-Line:** Uso de `remark` para identificação clara da política de segurança no arquivo de configuração.
...
* `Relatorio_ACL_InterRedes_Lab04.pdf`: Relatório técnico detalhado com comandos e evidências de verificação.
...
* **Verification:** Use of `show ip interface Gig0/2` to confirm the active filtering policy.
